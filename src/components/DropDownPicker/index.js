...
import UpArrowIcon from 'assets/icons/up-arrow-dropdown.svg';
...
class DropDownPicker extends React.Component {
    constructor(props) {
        ...
        if (!props.multiple) {
            if (props.defaultValue || props.defaultValue === 0) {
                choice = props.items.find(item => item[this.props.value] === props.defaultValue);
            ...
            defaultValueIndex = props.items.findIndex(item => item[this.props.value] === props.defaultValue);
        } else {
            if (props.defaultValue && Array.isArray(props.defaultValue) && props.defaultValue.length > 0) {
                props.defaultValue.forEach((value, index) => {
                    items.push(
                        props.items.find(item => item[this.props.value] === value)
                    )
                });
            } else if (props.items.filter(item => item.hasOwnProperty('selected') && item.selected === true).length > 0) {
                items = props.items.filter((item, index) => item.hasOwnProperty('selected') && item.selected === true);
            }
            defaultValueIndex = props.items.findIndex(item => item[this.props.value] === props.defaultValue[0]);
        }

        this.state = {
            choice: props.multiple ? items : {
                label: choice[this.props.label],
                value: choice[this.props.value],
                icon: choice.icon
            },
            searchableText: null,
    ...
    static getDerivedStateFromProps(props, state) {
        ...
        if (!state.props.multiple && props.defaultValue !== state.props.defaultValue) {
            const { label, value, icon } = props.defaultValue === null ? {
                ...
                value: null,
                icon: () => { }
            } : props.items.find(item => item[this.props.value] === props.defaultValue);
        ...
        this.setPropState({
            ...
            if (defaultValue) {
                if (this.state.props.multiple) {
                    ...
                    (async () => {
                        for (const value of defaultValue) {
                            await new Promise((resolve, reject) => {
                                resolve(
                                    this.select(items.find(item => item[this.props.value] === value))
                                );
                            });
                        }
                    })();
                    ...
                    this.select(
                        items.find(item => item[this.props.value] === defaultValue)
                    );
                }
            } else {
                this.reset();
    ...
    removeItem(value, { changeDefaultValue = true } = {}) {
        const items = [...this.props.items].filter(item => item[this.props.value] !== value);
        this.setPropState({
            items
        }, () => {
            if (changeDefaultValue) {
                if (this.state.props.multiple) {
                    ...
                    if (this.state.choice[this.props.value] === value) {
                        this.reset();
                    }
                }
            }
    ...
    selectItem(defaultValue) {
        if (this.state.props.multiple) {
            (async () => {
                for (const value of defaultValue) {
                    const item = this.props.items.find(item => item[this.props.value] === value);
                    if (item) {
                        await new Promise((resolve, reject) => {
                            resolve(
                                this.select(item)
            ...
            const item = this.props.items.find(item => item[this.props.value] === defaultValue);
            if (item)
                this.select(item);
        }
    }
    ...
    select(item) {
        ...
        if (!multiple) {
            ...
            const index = this.props.items.findIndex(i => i[this.props.value] === item[this.props.value]);

            // onChangeItem callback
            this.props.onChangeItem(item, index);
        } else {
            let choice = [...this.state.choice];
            const exists = choice.findIndex(i => i[this.props.label] === item[this.props.label] && i[this.props.value] === item[this.props.value]);

            if (exists > -1 && choice.length > this.props.min) {
                choice = choice.filter(i => i[this.props.label] !== item[this.props.label] && i[this.props.value] !== item[this.props.value]);
            ...
            // onChangeItem callback
            this.props.onChangeItem(choice.map(i => i[this.props.value]));
        }
    ...
    getItems() {
        if (this.state.searchableText) {
            ...
            return this.props.items.filter((item) => {
                return item[this.props.label] && (item[this.props.label].toLowerCase()).indexOf(text) > -1;
            });
        }
    ...
    isSelected(item) {
        return this.state.choice.findIndex(a => a[this.props.value] === item[this.props.value]) > -1;
    }
    ...
    getLabel(item, selected = false) {
        ...
        if (typeof item === 'object') {
            len = item[this.props.label].length;
            label = item[this.props.label].substr(0, selected ? this.props.selectedLabelLength : this.props.labelLength);
    ...
    render() {
        ...
        const isPlaceholderActive = this.state.choice[this.props.label] === null || (Array.isArray(this.state.choice) && this.state.choice.length === 0);
        const label = isPlaceholderActive ? (placeholder) : this.getLabel(this.state.choice[this.props.label], true);
            ...
            <View style={[this.props.containerStyle, {
                ...
                >
                    ...
                    <Text style={[
                        ...
                        this.state.choice[this.props.label] !== null && this.props.selectedLabelStyle,
                            ...
                            >
                                <TouchableOpacity
                                    ...
                                    style={[styles.dropDownItem, this.props.itemStyle, (
                                        this.state.choice[this.props.value] === item[this.props.value] && this.props.activeItemStyle
                                    ), {
                                ...
                                >
                                    <View style={{
                                        ...
                                        <Text style={[
                                            this.props.labelStyle,
                                            multiple ?
                                                (this.isSelected(item) && this.props.activeLabelStyle) : (this.state.choice[this.props.value] === item[this.props.value] && this.props.activeLabelStyle)
                                            , {
                                                ...(item.icon && {
                                                    marginHorizontal: 5
                                                })
                                    ...
                                    {
                                        this.state.props.multiple && this.state.choice.findIndex(i => i[this.props.label] === item[this.props.label] && i[this.props.value] === item[this.props.value]) > -1 && (
...
DropDownPicker.defaultProps = {
    ...
    customArrowUp: (size, color) => <UpArrowIcon width={Scale.moderateScale(15)} height={Scale.moderateScale(15)} />,
    customArrowDown: (size, color) => <DownArrowIcon width={Scale.moderateScale(15)} height={Scale.moderateScale(15)} />,
    customTickIcon: () => <Text>H</Text>,
    zIndex: 5000,
    disabled: false,
    searchable: false,
